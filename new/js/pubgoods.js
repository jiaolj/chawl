var Main = (function(){
	var _obj = {},
		...
		_isPub = 0,
		_ts = $('.twoStep'),
		_arg = {starting:'出发地',destination:'目的地',times:Base.tools.getStrTime({con:['-','-',' ','','','']}),leavewords:'',phone:''}, //'可装货时间'
		_get = function(){
	...
	return {
		...
		getTimes : function(){
			$('#timesbox').html(function(){
				...
				for(var i=0;i<60;i++){
					...
					htm += '<a><dt d="' + lst[0].trim() + '">' + lst[1] + '</dt></a>';
					date += 3600*24;
				}
				return htm;
		...
		init : function(){
			...
			$('.where a.auto').click(function(){
				var o = $(this);
				if(o.attr('state')=='1') o.attr('state','0');
				else o.attr('state','1');
			})
