...
var log = function(){for(var arg in arguments) console && console.log(arguments[arg])},
	...
	getUrl = function(){var l = location.href.split('/').pop(),r = l.split('?')[0];return r},
	getSize = function(){document.getElementById('html').style.fontSize = document.body.clientWidth*100/320+'px'}(),
	_isFollow = 0,
	_followFunc = function(){
		$('body').append('\
			<div class="isFollow" style="position:fixed;width:100%;height:100%;z-index:100;top:0;font-size:0.16rem;color:#fff">\
				<div style="position:absolute;width:100%;height:100%;background:rgba(0,0,0,0.5)"></div>\
				<div style="position:absolute;width:76%;top:1rem;left:12%;background:#df4b24;text-align:center;line-height:0.48rem;border-radius:0.08rem;padding:0.03rem">\
					<p>请关注查物流公众号</p>\
					<img style="width:70%;" src="img/pub/qrcode.jpg" />\
					<p>长按识别二维码</p>\
				</div>\
			</div>\
		')
	...
	_None = function(d){
		var c = parseFloat(d||0).toFixed(0);
		if(c==0) return '未知';
		return c;
	},
